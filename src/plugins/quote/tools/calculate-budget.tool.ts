...
import type { QuoteLineItem } from "../services/pdf.service.js";

const catalogService = new CatalogService();
const pdfService = new PdfService();

export const calculateBudgetTool = createTool({
  id: "calculateBudget",
...
Available items: grass types (m²), labor/mano de obra (m²), travel/desplazamiento (km).
Returns a formatted summary and a PDF attachment.`,
  ...
  inputSchema: z.object({
    ...
    items: z.array(
      z.object({
        nameOrCode: z.string().describe("Product name (partial match) or CODART number (1-8)"),
        quantity: z.number().positive().describe("Quantity in the item's unit (m² or km)"),
      })
    ).min(1),
    orgId: z.string().describe("Organization ID from the system context"),
  ...
  execute: async ({ clientName, clientAddress, items, orgId, applyVat }) => {
